  ...
  <a-card style="width: 100%">
    <div class="row mb-3">
      ...
      <div class="col-12 col-sm-12 mb-3">
        ...
        <a-form class="p-3" name="custom-validation" :label-col="{ span: 3 }"
          ...
          <div class="row">
            <div class="col-12 col-sm-12">
                      ...
                      <div class="editable-cell">
                        <div v-if="editableData[record.key]" class="editable-cell-input-wrapper">
                          <a-form-item label="ExpiredDate" required name="expiredDate">
                            <a-space direction="vertical" :size="12">
                              <a-date-picker v-model:value="editableData[record.key].expiredDate"
              ...
              <div class="col-12 col-sm-12 text-end">
                <h1></h1>
                <a-form-item :v-if="authStoreClaim['admin.requestdcdlabels.create'] === '1'" class="text-end">
...
export default defineComponent({
  ...
  setup() {
    ...
    // get data by ID
    //
    const getUsersEdit = (id) => {
      console.log('get user edit danh sach:', formState);
      ApiRequestDcdLabel.GetItemById(id).then((response) => {
        // gan form state
        formState.IdLine = `${response.data.idLine}`;
        formState.LineName = response.data.lineName;
        formState.RequestDate = response.data.requestDate;
        formState.TypeRequest = `${response.data.typeRequest}`;
        formState.ItemQuantity = response.data.itemQuantity;
        formState.RequestStatus = `${response.data.requestStatus}`;
        formState.Reason = response.data.reason;
        formState.CreatedBy = response.data.createdBy;
        formState.UpdatedBy = response.data.updatedBy;
        formState.ItemData = response.data.itemData;
        //
        dataSource.value = response.data.itemData;
        users.lastKeyTableRef = (response.data.itemData.length > 0) ? response.data.itemData[0].key : 0;
        //
      }).catch((error) => {
        console.log(error);
      });
    ...
    const createUsers = () => {
      ...
      //
      ApiRequestDcdLabel.EditItemById(formState).then((response) => {
