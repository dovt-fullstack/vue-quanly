    ...
    <div class="row mb-3">
      <div class="col-12">
        <a-breadcrumb>
          ...
          <a-breadcrumb-item>
            <router-link :to="{ name: 'admin-users' }">Store</router-link>
          </a-breadcrumb-item>
        ...
        <a-form class="p-3" name="custom-validation" ref="formRef" :model="formState" :rules="rules" v-bind="layout"
          ...
          <div class="row">
            <div class="col-12 col-sm-9">
              <a-form-item ref="userName" label="tên cửa hàng" required name="userName">
...
export default defineComponent({
  ...
  setup() {
    ...
    const createUsers = () => {
      ...
      axios.post('https://charismatic-friendship-production.up.railway.app/api/v1/admin/storetype/insert', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      }).then((response) => {
        message.success("Tạo mới thành công!");
        router.push({ name: "danh-sach-store" });
      }).catch((error) => {
        console.log(error);
      });
    }
